...
One Azure resource group holds everything:

- a **Function App** - the MCP server an AI connects to
- a **Key Vault** - the OAuth and bearer secrets
- a **managed identity** on the Function App - how it talks to Azure
- **App Insights + Log Analytics** - logs and metrics
- a **budget** - the cost cap

An AI client (Claude, ChatGPT, any MCP client) connects to the Function App over
OAuth, then calls tools. The tools operate that same resource group and the
node's GitHub repo. That is the whole loop: AI -> MCP server -> tools -> your
cloud and code.

## 1. The install

You paste one line into an agent CLI; it reads `install/agent.md` and runs the
deploy. Steps: check and install prerequisites, `az login`, create the resource
group, deploy `infra/node.bicep`, seed the OAuth secrets into the node's Key
Vault, publish the server from `src/`, then print your connect config.
...
One file, deployed into one resource group:

- **Storage** - required backing for a Function App.
- **Log Analytics + App Insights** - where logs and metrics go (with a daily
  ingestion cap as a cost knob).
- **Function App** on a Consumption plan - scale-to-zero, so it is ~$0 idle.
  It has a system-assigned managed identity.
- **Key Vault** in RBAC mode - holds the secrets.
- **Role assignments** - the node's identity gets Contributor on this resource
  group only, and Key Vault Secrets Officer on the vault; the installer also gets
  Secrets Officer so the first secret write succeeds (on an RBAC vault, being
...
- `functions/mcp.js` - the `/mcp` endpoint. Speaks MCP JSON-RPC (`initialize`,
  `tools/list`, `tools/call`); every call must carry the bearer.
- `lib/secrets.js` - reads Key Vault via the managed identity, short-TTL cached
  so a rotated secret is picked up quickly.
...
- **Node to Azure: managed identity.** No service principal, no secret to store
  ...
  resource group. That is why the node can manage its own cloud and nothing else.
...
- The AI is yours (Claude / ChatGPT), so there is no LLM cost to the node.
- The budget cap + alerts bound the resource group.
- New infra follows `kb/infra.md`: smallest sensible SKU, cost shown before
  deploy, clean naming + tags for easy teardown.

## 7. Scaling

One node to start. For another: re-run the install (new name, its own capped
RG), or run `scripts/enable-scaling.sh` once to grant the node subscription-scope
...
GitHub Action redeploys. The full lifecycle - fork, CI wiring, the update +
debug loop, and adding a tool's API key - is in `kb/extending.md` and
`kb/operating.md`, so the AI can follow it from inside the connector.
