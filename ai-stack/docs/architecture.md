...
- **The I-COST-1 knobs** (change a model or cut cost with no code change):
  | Knob | What it does |
  |---|---|
  | `MODEL_REASONER` | the produce-step model tier |
  | `MODEL_CHECKER` | the grade-step model tier (keep it cheap) |
  | `REASONER_MAX_ITERATIONS` | lower the loop ceiling (cannot raise above the spec cap) |
